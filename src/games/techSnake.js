...
function _tick() {
  ...
  if (head.x === _food.x && head.y === _food.y) {
    // Eat food
    _score += 10 + _food.tech.tier * 5;
    SoundManager.pop();
    _showTechPopup(_food.tech);

    if (!_collectedTechs.includes(_food.tech.name)) {
    ...
    EventBus.emit('xp:add', { amount: _food.tech.tier * 5, source: 'snake' });
...
function _draw() {
  ...
  // Food
  const foodTech = _food?.tech;
  _ctx.font = `${cellPx - 4}px serif`;
  _ctx.textAlign = 'center';
  _ctx.textBaseline = 'middle';
  if (foodTech) {
    _ctx.fillText(foodTech.icon, _food.x * cellPx + cellPx / 2, _food.y * cellPx + cellPx / 2);
  }

  // Snake body
  _snake.forEach((seg, i) => {
    const progress = 1 - i / _snake.length;
    _ctx.fillStyle = `rgba(${i === 0 ? '255, 62, 62' : '200, 60, 60'}, ${0.4 + progress * 0.6})`;
    _ctx.beginPath();
    _ctx.roundRect(
  ...
  _ctx.textAlign = 'left';
  _ctx.fillText(`Score: ${_score}`, 8, 16);
}

function _drawGameOver() {
  if (!_ctx) return;
  _ctx.fillStyle = 'rgba(13, 15, 23, 0.82)';
  _ctx.fillRect(0, 0, CANVAS_SIZE, CANVAS_SIZE);
  _ctx.fillStyle = '#fff';
  _ctx.font = 'bold 20px Outfit, sans-serif';
  _ctx.textAlign = 'center';
  _ctx.fillText('Game Over', CANVAS_SIZE / 2, CANVAS_SIZE / 2 - 20);
  _ctx.font = '14px Inter, sans-serif';
  _ctx.fillStyle = '#94a3b8';
  _ctx.fillText(`Score: ${_score} · High: ${SaveManager.get('snake').highScore}`, CANVAS_SIZE / 2, CANVAS_SIZE / 2 + 10);
  _ctx.fillText('Press R or click Restart', CANVAS_SIZE / 2, CANVAS_SIZE / 2 + 35);
}
...
function _updateScoreDisplay() {
  ...
  if (hi) hi.textContent = `Best: ${SaveManager.get('snake').highScore}`;
}
...
function _updateProgressSidebar() {
  ...
  list.innerHTML = CONFIG.snakeTechs.map(t => `
    <div class="snake-tech-item ${_collectedTechs.includes(t.name) ? 'collected' : ''}">
      <span>${t.icon}</span>
      <span class="snake-tech-name">${t.name}</span>
      ${_collectedTechs.includes(t.name) ? '<span class="snake-check">✓</span>' : ''}
    </div>
  `).join('');
}
...
function _showTechPopup(tech) {
  ...
  _popupTimeout = setTimeout(() => popup.classList.remove('snake-popup-show'), 2500);
}
...
function _bindKeys() {
  ...
  document.addEventListener('keydown', (e) => {
    if (_state !== 'playing') {
      if (e.key === 'r' || e.key === 'R') { e.preventDefault(); TechSnake.reset(); TechSnake.resume(); }
      return;
    }
